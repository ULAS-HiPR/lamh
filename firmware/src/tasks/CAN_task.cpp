...
void CAN_task::StartCAN() {
    ...
    for (;;) {
        ...
        if (has_pending_outbound_data_ &&
            ...
            send_canards_data(pending_outbound_data_);
...
bool CAN_task::process_rx_frame(const CAN_Frame& frame, flight_data& shared_data) {
    ...
    switch (frame.id) {
        ...
        case CAN_ID_IMU_GYRO: {
            ...
            shared_data.core_data.imu.gyro.x = payload.gx / 100.0f;
            shared_data.core_data.imu.gyro.y = payload.gy / 100.0f;
            shared_data.core_data.imu.gyro.z = payload.gz / 100.0f;
            shared_data.core_data.time = payload.timestamp_ms;
            return true;
        }
        ...
        case CAN_ID_BARO: {
            ...
            shared_data.core_data.barometer.altitude = payload.altitude / 10.0f;
            return true;
        }
...
void CAN_task::send_canards_data(const canards_raw& data) {
    CanardsPayload canards_payload{
        static_cast<int16_t>(data.kp * 100.0f),
        static_cast<int16_t>(data.kd * 100.0f),
        static_cast<int16_t>(data.servo_angle * 100.0f),
        data.active ? 1U : 0U
    };
    CAN_Frame canards_frame = pack_frame(CAN_ID_CANARDS, canards_payload);
    send_frame(canards_frame);
}
